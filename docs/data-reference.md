...
> **⚠️ This document is manually maintained and may drift from the actual data.** The Zod schemas in `src/lib/dtd/schemas/` are the authoritative source of truth for data structure. When in doubt, inspect the actual JSON files or run `bun run validate`.
