...
| **4A — Shared Modules**   | ✅     | `core.ts` + `dice.ts` ES module ports in `src/lib/dtd/`                                                                     |
...
| 4   | **combat-tracker**    | 2,204 | 0 (inline data)                         | Heavy core.ts usage (6+ APIs), cross-tool import from Sheet | ✅ Ported |
...
2. Convert `DTD.*` global calls → ES module imports from `@/lib/dtd/core.ts` and `@/lib/dtd/dice.ts`
...
### 5.2 — core.ts Coverage Gaps

The ES module port of `core.ts` exports 12 functions but not every function from the original is verified:
...
**Action:** When porting character-sheet, audit `core.ts` exports vs original tool usage, fix any gaps.
...
**Action:** Consider Vitest unit tests for `core.ts` and `dice.ts` ES modules.
...
5. ~~**Cleanup original tools/**~~ — ✅ `tools/` deleted after all ports verified
