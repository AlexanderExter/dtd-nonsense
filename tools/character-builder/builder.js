 ...
 * Depends on: DTD (core.js) for data loading, derived stat formulas,
...
const Builder = {
    ...
    async init() {
        try {
            this.data = await DTD.loadAllData([
        ...
        this.char = DTD.character.createDefault();
    ...
    bindEvents() {
        ...
        if (featSearch) {
            featSearch.addEventListener('input', DTD.debounce(() => this.renderFeats(), 250));
        }
    ...
    updateSidebar() {
        ...
        this._setStat('sidebar-sd', DTD.derived.calculateSD(chars.dexterity, chars.wisdom, size, isHalfling));
        this._setStat('sidebar-hp', DTD.derived.calculateHP(chars.constitution, chars.willpower));
        this._setStat('sidebar-md', DTD.derived.calculateMentalDefense(chars.composure));
        this._setStat('sidebar-resolve', DTD.derived.calculateResolve(chars.willpower, chars.composure));
        this._setStat('sidebar-speed', DTD.derived.calculateSpeed(chars.strength, chars.dexterity));
        this._setStat('sidebar-resilience', DTD.derived.calculateResilience(size, level));
        this._setStat('sidebar-initiative', DTD.derived.calculateInitiativeBase(chars.dexterity, chars.composure));
    ...
    renderReview() {
        ...
        // Derived stats
        const sd = DTD.derived.calculateSD(chars.dexterity, chars.wisdom, size, isHalfling);
        ...
        const resolve = DTD.derived.calculateResolve(chars.willpower, chars.composure);
        const speed = DTD.derived.calculateSpeed(chars.strength, chars.dexterity);
        const resilience = DTD.derived.calculateResilience(size, level);
        const init = DTD.derived.calculateInitiativeBase(chars.dexterity, chars.composure);
    ...
    buildExportData() {
        ...
        return DTD.character.validate(this.char);
    },

    exportJSON() {
        const data = this.buildExportData();
        DTD.character.exportJSON(data);
    },

    openInSheet() {
        const data = this.buildExportData();
        // Save to shared storage so the Sheet can pick it up
        const list = DTD.character.list();
        if (!list.find(c => c.id === data.id)) {
            list.push({ id: data.id, name: data.name || 'Builder Character' });
            localStorage.setItem(DTD.character.STORAGE_LIST_KEY, JSON.stringify(list));
        }
        DTD.character.save(data.id, data);
        // Navigate to Sheet
        window.location.href = '../character-sheet/index.html';
    },
