...
const Builder = {
	...
	bindEvents() {
		...
		if (featSearch) {
			featSearch.addEventListener(
				"input",
				debounce(() => this.renderFeats(), 250),
			);
		}
	...
	updateAllSummaries() {
		...
		// Step 9: Feats
		const featTotal =
			(this.char.feats?.length || 0) + (this.char.assets?.length || 0) + (this.char.hindrances?.length || 0);
	...
	renderCharPriorities() {
		...
		if (!cont) return;
		cont.innerHTML = Object.entries(this.CHAR_GROUPS)
			.map(([gk, g]) => {
	...
	updateCharSummary() {
		...
		const maxTotal = Object.values(this.meta.charPriority).reduce(
			(sum, p) => sum + (p ? this.CHAR_PRIORITY_DOTS[p] : 0),
			0,
		);
	...
	updateSkillSummary() {
		...
		const maxTotal = Object.values(this.meta.skillPriority).reduce(
			(sum, p) => sum + (p ? this.SKILL_PRIORITY_DOTS[p] : 0),
			0,
		);
	...
	showClassDetail(classId) {
		...
		const prereqHtml =
			cls.prerequisites && cls.prerequisites !== "None"
	...
	renderReview() {
		...
		const bgsHtml =
			(this.char.backgrounds || [])
				.filter((b) => b.dots > 0)
				.map((b) => `<li>${this.esc(b.name)}: ${b.dots} dot${b.dots > 1 ? "s" : ""}</li>`)
				.join("") || "<li>None</li>";
		...
		el.innerHTML = `
            ...
            <div class="review-section">
                ...
                <ul class="review-list">
                    ${
						Object.entries(this.char.skills || {})
							.filter(([, v]) => v > 0)
							.map(([id, v]) => {
								const sk = ["physical", "social", "mental"]
									.flatMap((g) => this.data.skills?.skills?.[g] || [])
									.find((s) => s.id === id);
            ...
            <div class="review-section">
                ...
                ${Object.entries(xp.breakdown)
					.map(([k, v]) =>
						v ? `<div class="stat-row"><span>${this.capitalize(k)}</span><span>${v}</span></div>` : "",
					)
					.join("")}
