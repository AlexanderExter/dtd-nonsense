...
const Sheet = {
	...
	async loadGameData() {
		...
		this.data = await loadAllData(files);
	},
	...
	updateDerivedDisplay() {
		...
		if (sdFormulaEl) {
			sdFormulaEl.textContent =
				this.char.race === "halfling" ? "10 + Dex × 6 − Size × 2" : "10 + (Dex + Wis) × 3 − Size × 2";
		}
	...
	updateWoundStatus(maxHP, currentHP) {
		...
		if (hpLost <= 0) {
			...
			description =
				"At 0 HP, further hits cause Critical Damage. Consult Critical Effects Tables by hit location and damage type. Critical Damage does not heal naturally — requires medical attention (1 point/week with rest and care).";
	...
	renderAlignmentInfo() {
		...
		if (align.sins?.length) {
			...
			for (const entry of align.sins) {
				const highlight =
					this.char.devotion && entry.devotion === this.char.devotion ? ' class="sin-current"' : "";
                ...
                <div class="pool-calc-grid">
                    <div class="pool-calc-field">
                        ...
                        <select id="pool-calc-char" class="pool-calc-select">
                            <option value="">—</option>
                            ${Object.entries(this.CHAR_NAMES)
	...
	updatePoolCalcResult() {
		...
		if (!charId) {
			...
			if (specHint) specHint.style.display = "none";
			return;
		}
	...
	renderArmorList() {
		...
		items.forEach((a, i) => {
			const typeOpts = this.ARMOR_TYPES.map(
	...
	renderMeleeList() {
		...
		items.forEach((w, i) => {
			...
			const parsed = parseNotation(dmgStr);
			let totalDmg = dmgStr;
			if (parsed) {
				totalDmg = `${parsed.num + eff.strength}k${parsed.keep}`;
			}

			const typeOpts = this.WEAPON_TYPES_MELEE.map(
				(t) => `<option value="${t}" ${w.weaponType === t ? "selected" : ""}>${t}</option>`,
			).join("");
			const profOpts = this.PROFICIENCY_MELEE.map(
				(p) => `<option value="${p}" ${w.proficiency === p ? "selected" : ""}>${p}</option>`,
			).join("");
			const dmgTypeOpts = this.DAMAGE_TYPES.map(
				(t) => `<option value="${t}" ${w.damageType === t ? "selected" : ""}>${t}</option>`,
			).join("");
			const availOpts = this.AVAILABILITY.map(
				(a) => `<option value="${a}" ${w.availability === a ? "selected" : ""}>${a}</option>`,
			).join("");
	...
	renderRangedList() {
		...
		items.forEach((w, i) => {
			const typeOpts = this.WEAPON_TYPES_RANGED.map(
				(t) => `<option value="${t}" ${w.weaponType === t ? "selected" : ""}>${t}</option>`,
			).join("");
			const profOpts = this.PROFICIENCY_RANGED.map(
				(p) => `<option value="${p}" ${w.proficiency === p ? "selected" : ""}>${p}</option>`,
			).join("");
			const dmgTypeOpts = this.DAMAGE_TYPES.map(
				(t) => `<option value="${t}" ${w.damageType === t ? "selected" : ""}>${t}</option>`,
	...
	renderMagicSchools() {
		...
		return this.MAGIC_SCHOOLS.map((school) => {
			...
			const bonusDisabled =
				extraSchoolLevels > 0 ? 'disabled style="opacity:0.4;width:36px;font-size:0.78rem"' : "";
	...
	renderClassDetails(cls) {
		...
		if (cls.feats?.length) {
			featsHtml =
				...
				cls.feats
					...
					.join("") +
				'</ul><p class="feat-legend">* = optional</p>';
		}
		...
		if (cls.completionBonus)
			html += `<p><span class="info-label">Completion:</span> ${this.esc(cls.completionBonus)}</p>`;
		if (cls.suggestedExits?.length)
			html += `<p><span class="info-label">Exits:</span> ${this.esc(cls.suggestedExits.join(", "))}</p>`;
	...
	bindEvents() {
		...
		for (const [id, field] of [
			["track-hp", "currentHP"],
			["track-resource", "resourceCurrent"],
			["track-resolve", "currentResolve"],
	...
	handlePanelInput(e) {
		...
		if (el.dataset.field) {
			...
			if (["heroPointsMax", "heroPointsBurnt"].includes(field)) {
				this.setText(
					"disp-hero-max",
					Math.max(0, (this.char.heroPointsMax || 2) - (this.char.heroPointsBurnt || 0)),
				);
			}
