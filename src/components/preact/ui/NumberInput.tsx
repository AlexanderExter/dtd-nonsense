...
interface NumberInputProps extends Omit<JSX.HTMLAttributes<HTMLInputElement>, "type" | "size"> {
	width?: InputWidth;
	label?: string;
}
...
export function NumberInput({ width = "sm", label, class: cls, id, ...rest }: NumberInputProps) {
	const inputCls = [BASE_CLS, WIDTH_CLS[width], cls].filter(Boolean).join(" ");

	if (label) {
		return (
			<label class="flex flex-col gap-[2px] m-0">
				<span class="text-[0.7rem] uppercase tracking-[0.5px] text-text-dim font-semibold">{label}</span>
				<input type="number" id={id} class={inputCls} {...rest} />
			</label>
		);
	}

	return <input type="number" id={id} class={inputCls} {...rest} />;
}
