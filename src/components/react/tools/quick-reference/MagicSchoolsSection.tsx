...
function WarpTable({ title, entries }: { title: string; entries: readonly QRefWarpEntry[] }) {
	return (
		<details className="mt-md border border-border rounded-md overflow-hidden">
			<summary className="px-md py-sm bg-surface cursor-pointer text-accent font-semibold text-[0.9rem] hover:bg-surface-highlight select-none">
				{title}
			</summary>
			<div className="overflow-x-auto max-h-[400px] overflow-y-auto">
				<table className="w-full border-collapse text-[0.82rem]">
					<thead className="sticky top-0">
						<tr>
							<th className="px-md py-xs text-left border-b border-border bg-surface text-text-muted font-semibold text-[0.78rem] uppercase tracking-[0.5px] w-[70px]">
								Roll
							</th>
							<th className="px-md py-xs text-left border-b border-border bg-surface text-text-muted font-semibold text-[0.78rem] uppercase tracking-[0.5px] w-[160px]">
								Effect
							</th>
							<th className="px-md py-xs text-left border-b border-border bg-surface text-text-muted font-semibold text-[0.78rem] uppercase tracking-[0.5px]">
								Description
							</th>
						</tr>
					</thead>
					<tbody>
						{entries.map((e) => (
							<tr key={e.roll} className="even:bg-stripe hover:bg-surface">
								<td className="px-md py-xs border-b border-border font-mono text-[0.8rem]">{e.roll}</td>
								<td className="px-md py-xs border-b border-border font-semibold text-accent">
									{e.name}
								</td>
								<td className="px-md py-xs border-b border-border text-text-muted">{e.effect}</td>
							</tr>
						))}
					</tbody>
				</table>
		...
		<>
			<div className="overflow-x-auto">
				<table className="w-full border-collapse text-[0.88rem] max-[600px]:text-[0.8rem]">
					...
					<tbody>
						{filtered.map((m) => (
							<tr key={m.school} className="even:bg-stripe hover:bg-surface">
			...
			<div className="grid grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-md mt-md max-[600px]:grid-cols-1">
				<div className="bg-surface border border-border rounded-sm p-md">
					<h4 className="text-accent m-0 mb-xs text-[0.9rem]">Fettered</h4>
					<p className="m-0 text-[0.85rem] text-text-muted">Halve rolled dice. No Psychic Phenomena risk.</p>
				</div>
				<div className="bg-surface border border-border rounded-sm p-md">
					<h4 className="text-accent m-0 mb-xs text-[0.9rem]">Unfettered</h4>
					<p className="m-0 text-[0.85rem] text-text-muted">
						Full dice. If keeping exploded 10s, roll Psychic Phenomena.
					</p>
				</div>
				<div className="bg-surface border border-border rounded-sm p-md">
					<h4 className="text-accent m-0 mb-xs text-[0.9rem]">Push</h4>
					<p className="m-0 text-[0.85rem] text-text-muted">
