...
export function StatCard({ npc, derivedStats, traitsData }: StatCardProps) {
	...
	return (
		<div className="stat-card flex-1 rounded-md border-2 border-border bg-surface p-lg">
			...
			<table className="m-0 mb-md w-auto">
				<thead>
					<tr>
						{CHAR_KEYS.map((key) => (
							<th
								className="min-w-[36px] border-border border-b bg-transparent px-sm py-xs text-center text-text-muted text-xs"
								key={key}
							>
								{CHAR_ABBREV[key]}
							</th>
				...
				<tbody>
					<tr>
						{CHAR_KEYS.map((key) => {
							...
							return (
								<td
									className="min-w-[36px] border-b-0 px-sm py-xs text-center font-bold text-sm"
									key={key}
								>
									{val === 0 ? "-" : val}
								</td>
			...
			{/* Stats line */}
			<div className="mb-md text-sm text-text-primary leading-[1.8]">
